		...
		<html>
			...
			<body>
				...
				<form name="formulario" method="POST" action="updMenu">
					...
					<input name="id_usuario" type="hidden" value = "{Cabecera/Parametros/upd_id_usuario}"/>
						...
						<div class="divtbl">
							<table>
								<tr>
									...
									<td class="td-c1">
										<input type="text" name="nombre_usuario" value="{Cuerpo/usuario/nombre_usuario}"/>
									</td>
								</tr>
								<tr>
										...
										<select name="perfil">
											...
											<xsl:for-each select="Cuerpo/listaPerfil/fila">
												<option value="{id_perfil}">
													<xsl:if test="id_perfil = /Documento/Cuerpo/usuario/id_perfil">
